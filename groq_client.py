...
def _needs_period_clarification(messages: list[dict]) -> bool:
    """Recognize an affirmative answer to Skew's optional-period question."""
    if len(messages) < 2:
        return False
    latest = messages[-1]
    previous = messages[-2]
    if latest.get("role") != "user" or previous.get("role") != "assistant":
        return False
    answer = str(latest.get("content", "")).strip().lower().rstrip(".!?")
    affirmative = answer in {
        "yes", "yes please", "yes i would", "yeah", "yep", "sure", "please do"
    }
    prior_question = "would you like me to test a different period" in str(
        previous.get("content", "")
    ).lower()
    return affirmative and prior_question


def _recover_failed_tool_arguments(error_payload: dict) -> dict | None:
    """Recover valid JSON arguments from Groq's malformed XML-like call."""
    error = error_payload.get("error") or {}
    if error.get("code") != "tool_use_failed":
        return None
    failed = str(error.get("failed_generation") or "")
    match = re.search(
        r"(?:function=)?run_skew_verdict\s*(\{.*?\})\s*</function>",
        failed,
        flags=re.DOTALL,
    )
    if not match:
...
def chat_turn(messages: list[dict]) -> tuple[str, list[dict]]:
    ...
    """
    if _needs_period_clarification(messages):
        reply = (
            "What period would you like me to test? For example, you can say "
            "**January 2020 to December 2022**, **since 2023**, or "
            "**the last two years**."
        )
    ...
    for _ in range(4):  # cap tool-call round-trips so a loop can't run forever
        ...
        if not resp.ok:
            try:
                ...
                error_payload = {}
            recovered_args = _recover_failed_tool_arguments(error_payload)
            if recovered_args:
                recovered_result = _run_tool(
                    claim_text=recovered_args.get("claim_text", ""),
                    expected_direction=recovered_args.get("expected_direction"),
                )
                formatted = _format_statistical_result(recovered_result)
                if formatted:
